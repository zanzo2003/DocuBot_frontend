...
import { Upload, FileText, Trash2, Send, Moon, Sun, MessageCircle, Bot, User, AlertCircle, CheckCircle, X, GripVertical } from 'lucide-react';
import { marked } from 'marked'; // Import marked for markdown rendering
...
const ResizableDivider = ({ onDrag, darkMode, isResizing }) => {
  return (
    <div 
      className={`w-1 bg-gray-300 hover:bg-blue-400 cursor-col-resize flex items-center justify-center transition-colors duration-200 relative group ${
        darkMode ? 'bg-gray-600 hover:bg-blue-500' : ''
    ...
    >
      <div className={`absolute inset-y-0 -left-1 -right-1 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 ${
        ...
        <GripVertical size={16} className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`} />
      </div>
    </div>
  );
};
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      >
        {/* Left Panel - File Upload & Preview */}
        <div 
          className={`transition-colors duration-300 ${darkMode ? 'bg-gray-800' : 'bg-white'} overflow-hidden`}
        ...
        {/* Right Panel - Chat */}
        <div 
          className={`flex flex-col transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-white'} overflow-hidden`}
